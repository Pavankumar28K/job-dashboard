...
function renderDetail() {
  ...
  detailPanel.innerHTML = `<div class="detail-head">
    ...
    <div class="recruiter-card">
      ...
      <div class="detail-grid">
        <div class="field"><label>Name</label><input id="recruiterNameInput" class="input" value="${escapeHtml(job.recruiterName || "")}" placeholder="Recruiter name" /></div>
        <div class="field"><label>Email</label><input id="recruiterEmailInput" class="input" value="${escapeHtml(job.recruiterEmail || "")}" placeholder="recruiter@company.com" /></div>
        <div class="field"><label>Phone</label><input id="recruiterPhoneInput" class="input" value="${escapeHtml(job.recruiterPhone || "")}" placeholder="+1 (555) 000-0000" /></div>
      </div>
    </div>
    ...
    <div class="detail-actions">
      ...
      <button class="button warning" id="generateResume">Generate Resume</button>
      <button class="button warning" id="generateCover">Generate Cover</button>
...
const RESUME_VERSIONS = [
  {
    key: "angular",
    label: "Angular + Azure",
    file: "Pavan_Konatham_NET_Azure_Angular.docx",
    color: "#1f78d1",
    keywords: [
      "angular","ngrx","rxjs","reactive forms","signals","spa","component library","scss","bootstrap",
      "azure","aks","azure functions","service bus","blob storage","azure sql","azure openai","managed identity",
      "application insights","log analytics","azure devops","azure ad",
      ".net","dotnet","c#","asp.net","entity framework","ef core","web api","signalr","grpc","wcf",
      "sql server","t-sql","stored procedures","ssrs","ssis","postgresql","redis",
      "jwt","oauth","rbac","clean architecture","tdd","agile","scrum","microservices",
      "docker","kubernetes","helm","kafka","ci/cd","git",
    ],
  },
  {
    key: "aws",
    label: "AWS + React",
    file: "Pavan_Konatham_NET_AWS_React.docx",
    color: "#e47911",
    keywords: [
      "react","redux","react query","hooks","context api","tailwind","typescript","next.js",
      "aws","lambda","api gateway","s3","dynamodb","elasticache","ecs","eks","cloudwatch",
      "iam","sqs","sns","secrets manager","rds","terraform","github actions","codepipeline","codebuild",
      ".net","dotnet","c#","asp.net","entity framework","ef core","web api","signalr",
      "sql server","t-sql","stored procedures","postgresql","redis","nosql",
      "jwt","oauth","rbac","microservices","clean architecture","tdd","agile","scrum",
      "docker","kubernetes","helm","kafka","ci/cd","git",
    ],
  },
  {
    key: "ai",
    label: "Agentic AI + Azure",
    file: "Pavan_Konatham_NET_Azure_AgenticAI.docx",
    color: "#7c3aed",
    keywords: [
      "ai","llm","gpt","gpt-4","openai","azure openai","generative ai","genai","rag","retrieval",
      "embedding","vector","vector search","langchain","autogen","semantic kernel","prompt engineering",
      "multi-agent","agentic","ai agent","function calling","tool use","ai foundry","cognitive search",
      "faiss","nlp","machine learning","deep learning","pytorch","tensorflow","copilot","ai pipeline",
      "azure","aks","azure functions","service bus","blob storage","azure sql","managed identity",
      "application insights","azure devops",
      ".net","dotnet","c#","asp.net","entity framework","web api","angular","signalr",
      "sql server","postgresql","redis","kafka","docker","kubernetes","ci/cd",
    ],
  },
];

function computeResumeMatch(job) {
  const blob = [job.role, job.company, job.notes, job.jd, job.workAuthRisk]
    .filter(Boolean).join(" ").toLowerCase();
  return RESUME_VERSIONS.map((rv) => {
    const hits = rv.keywords.filter((kw) => blob.includes(kw)).length;
    const pct = Math.min(100, Math.round((hits / rv.keywords.length) * 100 * 2.5)); // scale up so 40% keyword hit = 100%
    return { ...rv, hits, pct };
  });
}

function computeMissingSkills(topN = 30) {
  // All keywords across all 3 base resumes (lowercased set)
  const resumeKwSet = new Set(
    RESUME_VERSIONS.flatMap((rv) => rv.keywords.map((k) => k.toLowerCase()))
  );
  // Extra explicit resume terms not in keyword bags
  const extraResumeTerms = [
    "c#","asp.net",".net","entity framework","ef core","web api","signalr","grpc","wcf",
    "sql server","t-sql","stored procedures","ssrs","ssis","angular","rxjs","ngrx",
    "azure openai","azure devops","azure ad","azure","aws","docker","kubernetes","kafka",
    "jwt","oauth","rbac","microservices","clean architecture","tdd","agile","scrum",
    "git","helm","ci/cd","postgresql","redis","dynamodb","typescript","python","react",
    "langchain","autogen","semantic kernel","prompt engineering","rag","openai","faiss",
    "vector search","embedding","llm","gpt","multi-agent","agentic","terraform",
    "github actions","cloudwatch","secrets manager","tailwind","next.js","graphql",
  ];
  extraResumeTerms.forEach((t) => resumeKwSet.add(t.toLowerCase()));

  const skillsData = computeSkillsGap().slice(0, topN);
  return skillsData
    .filter(({ skill }) => {
      const sl = skill.toLowerCase();
      // missing if no resume keyword contains or equals this skill
      return ![...resumeKwSet].some((kw) => kw === sl || kw.includes(sl) || sl.includes(kw));
    })
    .slice(0, 20); // show up to 20 missing skills
}
...
function computeSkillsGap() {
  ...
  for (const [cat, skills] of Object.entries(SKILLS_DICT)) {
    for (const skill of skills) {
      ...
      const count = jobs.filter((j) => re.test((j.jd || "") + " " + (j.role || "") + " " + (j.notes || ""))).length;
  ...
  analyticsPanel.innerHTML = /* html */`
    ...
    <div class="analytics-grid">
      ...
      <div class="analytics-card" id="missingSkillsCard">
        ...
        <p style="font-size:12px;color:var(--text-muted);margin:0 0 12px">Top demanded skills from your job pipeline that aren't covered by any of your base resumes.</p>
        ${(() => {
          const missing = computeMissingSkills();
          if (!missing.length) return `<div class="empty-state" style="padding:10px 0">Your resumes cover all top demanded skills 🎉</div>`;
...
const TITLE_SKILL_KB = {
  ...
  "angular":      { must: ["Angular","TypeScript","RxJS","HTML","CSS","Git"], nice: [".NET","REST API","NgRx","Jest","Azure DevOps"] },
  // Backend
  "backend":      { must: ["REST API","SQL","Git","OOP"], nice: ["Node.js","C#",".NET","Docker","Redis","Microservices","Azure"] },
  "back end":     { must: ["REST API","SQL","Git","OOP"], nice: ["Node.js","C#",".NET","Docker","Redis","Microservices","Azure"] },
  ...
  "azure":        { must: ["Azure","CI/CD","Docker","Git"], nice: ["Kubernetes","Azure DevOps","TypeScript","C#",".NET","Terraform"] },
  // Software engineer / developer (generic)
  "software engineer": { must: ["OOP","Git","SQL","REST API","Data Structures"], nice: ["C#","Python","JavaScript","Docker","Azure","TypeScript"] },
  "software developer":{ must: ["OOP","Git","SQL","REST API","Data Structures"], nice: ["C#","Python","JavaScript","Docker","Azure","TypeScript"] },
  "developer":    { must: ["OOP","Git","SQL","REST API"], nice: ["C#","JavaScript","Python","Docker","Azure","TypeScript"] },
  "engineer":     { must: ["OOP","Git","SQL","REST API"], nice: ["C#","JavaScript","Python","Docker","Azure","TypeScript"] },
  // Microservices / architecture
  "microservices":{ must: ["Microservices","REST API","Docker","SQL","Git"], nice: ["Kubernetes","Azure","Redis","C#",".NET","RabbitMQ","CQRS"] },
  "architect":    { must: ["OOP","REST API","SQL","Microservices","Cloud"], nice: ["Azure","Docker","Kubernetes","C#","TypeScript","CI/CD"] },
  // Data
  "data":         { must: ["SQL","Python","Data Analysis","Git"], nice: ["Azure","Power BI","ETL","Spark","Machine Learning","TypeScript"] },
  "analyst":      { must: ["SQL","Excel","Data Analysis"], nice: ["Python","Power BI","Azure","Tableau","REST API"] },
};

function suggestSkillsFromTitles() {
  const titlesText = ($("#settingsTitles").value || "").toLowerCase();
  ...
  for (const [keyword, skills] of Object.entries(TITLE_SKILL_KB)) {
    if (titlesText.includes(keyword)) {
...
function applySkillSuggestions(type) {
  ...
  if (type === "must") {
    ...
    toast("Must-have skills applied ✓");
  } else {
    ...
    toast("Nice-to-have skills applied ✓");
  }
}
...
(function initPanelResize() {
  ...
  if (saved) detPanel.style.flex = `0 0 ${saved}px`;
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    const newW = Math.min(Math.max(startW + delta, 200), layoutW - 320);
...
const SKILL_META_RM = {
  ...
  "JavaScript":    { phase:1, label:"JavaScript",     icon:"⚡", desc:"ES6+, async/await, DOM, fetch API" },
        ...
        topics: [
          {
            ...
            note: "Merge Sort: O(n log n) stable. Quick Sort: O(n log n) average, O(n²) worst. Know both.",
            problems: [
...
function prepStreakBanner() {
  ...
  const todayDot = activeToday
    ? `<span style="color:#10b981;font-size:12px;font-weight:600">✓ Active today</span>`
...
function renderRoadmap() {
  ...
  function projectsBlock(idx) {
    const pd = PHASE_PROJECTS_RM[idx];
    if (!pd) return "";
    const diffColor = { "Beginner":"#2e7d32","Intermediate":"#1565c0","Advanced":"#c62828" };
    const cards = pd.projects.map(pr => `
      <div class="prj-card">
        <div class="prj-top">
          <span class="prj-diff" style="background:${diffColor[pr.difficulty]||"#555"}">${pr.difficulty}</span>
          <div class="prj-skills">${pr.skills.map(s=>`<span class="prj-skill-tag">${s}</span>`).join("")}</div>
        </div>
        <div class="prj-title">${pr.title}</div>
        <div class="prj-desc">${pr.desc}</div>
        <ul class="prj-highlights">${pr.highlights.map(h=>`<li>${h}</li>`).join("")}</ul>
  ...
  function phaseBlock(p, idx) {
    ...
    return `
      ...
      ${projectsBlock(idx)}
      ${idx < phaseKeys.length-1 ? '<div class="rmp-arrow">↓</div>' : ""}`;
  }
  ...
  function renderModal() {
    ...
    document.getElementById("skillModalInner").innerHTML = `
      <div style="background:linear-gradient(135deg,${color}ee,${color}88);color:#fff;padding:28px 32px 20px">
        <button id="skillModalClose" style="float:right;background:rgba(255,255,255,.25);border:none;color:#fff;font-size:18px;width:32px;height:32px;border-radius:50%;cursor:pointer;line-height:1">✕</button>
